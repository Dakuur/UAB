num1 = float(input("Introdueix num 1: "))
num2 = float(input("Introdueix num 2: "))

i = True

resultat = 0

while i == True:

    operacio = int(input("MENÚ\n1.- Suma\n2.- Resta\n3.- Producte\n4.- Divisió\n5.- Sortir\nSelecciona una de les opcions: "))

    if operacio == 5:

        resposta = str.upper(input("Vols operar amb uns altres operands? (S/-)"))

        if resposta == "S":

            num1 = float(input())
            num2 = float(input())
        
        else:

            i = False
    
    else:

        operacions = {
            1 : "+",
            2 : "-",
            3 : "x",
            4 : ":",
        }

        if operacio < 1 or operacio > 5:
            print("Error: Opció no vàlida")
        elif operacio == 5:
                print("Sortint de la calculadora...")
        else:

            if operacio == 4 and num2 == 0:
                print("Error: Divisió per zero")
            else:
                if operacio == 1:
                    resultat = num1 + num2
                elif operacio == 2:
                    resultat = num1 - num2
                elif operacio == 3:
                    resultat = num1 * num2
                elif operacio == 4:
                    resultat = num1 / num2

                print(num1, operacions[operacio], num2, "=", resultat)